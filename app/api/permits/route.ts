...
const PermitsQuerySchema = z.object({
  ...
  status: z.enum(["PENDING", "APPROVED", "HALTED", "CANCELLED", "EXPIRED", "COMPLETED"]).optional(),
});
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: vessels } = await supabase
      ...
      .in(
        "id",
        permits.filter((p) => p.vessel_id).map((p) => p.vessel_id)
      );

    // Create maps for quick lookup
    const driverMap: Record<string, any> = drivers?.reduce((acc: Record<string, any>, d) => ({ ...acc, [d.id]: d }), {}) || {};
    const slotMap: Record<string, any> = slots?.reduce((acc: Record<string, any>, s) => ({ ...acc, [s.id]: s }), {}) || {};
    const vesselMap: Record<string, any> = vessels?.reduce((acc: Record<string, any>, v) => ({ ...acc, [v.id]: v }), {}) || {};
